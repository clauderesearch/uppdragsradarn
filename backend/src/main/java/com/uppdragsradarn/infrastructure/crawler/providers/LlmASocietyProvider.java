...
/**
 * Simplified LLM-based provider for A Society Group.
 * Uses Playwright only for getting job URLs, then LLM for detail extraction.
 */
@Component
@Slf4j
public class LlmASocietyProvider extends AbstractProvider {
    ...
    protected List<Assignment> fetchAndParse(Source source) throws CrawlerException {
        ...
        try {
            ...
            logger.info("Successfully extracted {} assignments from A Society Group using LLM", assignments.size());
            return assignments;
    ...
    /**
     * Simplified job URL fetching using Playwright.
     * Only extracts URLs, no complex data extraction.
     */
