...
/**
 * Provider for Konsultfabriken job listings.
 * Simple PHP-based job board with straightforward HTML structure.
 */
@Component
@Slf4j
public class KonsultfabrikenProvider extends AbstractHttpProvider {
  ...
  private Assignment extractAssignmentFromLink(Element jobLink, Source source) {
    ...
    if (publishDate != null) {
      assignment.setPublishedDate(publishDate);
    }
    ...
    try {
      ...
      if (content != null && !content.isEmpty()) {
        ...
        if (!detailedLocation.equals("Sweden")) {
          locationService.processAssignmentLocation(assignment, detailedLocation, assignment.getSource().getName());
        }
  ...
  private String extractLocationFromTitle(String title) {
    ...
    if (lowercaseTitle.contains("göteborg") || lowercaseTitle.contains("gothenburg")) return "Göteborg";
  ...
  private String extractLocationFromContent(String content) {
    ...
    if (lowercaseContent.contains("remote work") || lowercaseContent.contains("distansarbete") ||
        lowercaseContent.contains("work from home") || lowercaseContent.contains("hemarbete")) {
      return "Remote";
    }
  ...
  private Set<String> extractSkillsFromContent(String content) {
    ...
    String[] techSkills = {
        ...
        "c#", ".net", "asp.net", "azure", "aws", "gcp", "cloud",
        ...
        "linux", "windows", "security", "testing", "automation"
    };
  ...
  private void extractAndSetRate(Assignment assignment, String content) {
    ...
    Pattern ratePattern = Pattern.compile("(\\d{3,4})\\s*(-\\s*(\\d{3,4}))?\\s*(kr|SEK|per timme|/h|/tim)", Pattern.CASE_INSENSITIVE);
  ...
  private com.uppdragsradarn.domain.model.Currency findOrCreateCurrency(String code) {
    return currencyRepository
        .findByCode(code)
        .orElseGet(() -> {
          var currency = com.uppdragsradarn.domain.model.Currency.builder()
              .code(code)
              .name("Swedish Krona")
              .symbol("kr")
              .build();
