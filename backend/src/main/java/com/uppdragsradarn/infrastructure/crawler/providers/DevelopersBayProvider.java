...
/**
 * Provider for Developers Bay job listings.
 * Developers Bay is a freelance community that posts public assignments.
 */
@Component
@Slf4j
public class DevelopersBayProvider extends AbstractHttpProvider {

  private static final String LISTINGS_URL = "https://developersbay.se/tillgangliga-offentliga-uppdrag/";
  ...
  private Assignment fetchAndExtractFromUrl(String url, Source source) {
    try {
      ...
      // Extract content
      Element contentElement = doc.selectFirst(".entry-content, .elementor-widget-theme-post-content");
  ...
  private void enrichAssignmentFromDetailPage(Assignment assignment) {
    try {
      ...
      if (!"Sweden".equals(location)) {
        locationService.processAssignmentLocation(assignment, location, assignment.getSource().getName());
      }
  ...
  private String cleanDescription(Element contentElement) {
    ...
    String text = contentElement.wholeText()
        .replaceAll("\\s+", " ")
        .trim();
  ...
  private Set<String> extractSkillsFromContent(Document doc) {
    ...
    String[] techSkills = {
        ...
        "spring", "nodejs", "aws", "azure", "kubernetes", "docker", "devops",
        "ci/cd", "agile", "scrum", "sql", "nosql", "mongodb", "postgresql",
        "git", "jenkins", "terraform", "ansible", "linux", "windows",
        "microservices", "rest", "api", "frontend", "backend", "fullstack"
    };
