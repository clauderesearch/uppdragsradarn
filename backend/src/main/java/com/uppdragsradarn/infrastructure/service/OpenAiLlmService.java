...
/**
 * Service for integrating with OpenAI's GPT-4o-mini model for job detail extraction. Provides async
 * and sync methods for processing HTML content into structured assignment data.
 */
@Service
  ...
  public static class ExtractedAssignmentData {
    ...
    public void applyToAssignment(Assignment assignment) {
      ...
      if (hourlyRateMin != null)
        assignment.setHourlyRateMin(new java.math.BigDecimal(hourlyRateMin));
      if (hourlyRateMax != null)
        assignment.setHourlyRateMax(new java.math.BigDecimal(hourlyRateMax));
