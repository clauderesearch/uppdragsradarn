...
/**
 * Service for filtering and optimizing HTML content before sending to LLM.
 * Removes unnecessary elements, limits token count, and applies readability algorithms.
 */
@Service
public class HtmlContentFilterService {
    ...
    public String filterContent(String htmlContent, ExtractionConfig config) {
        ...
        try {
            ...
            logger.debug("Filtered content from {} to {} characters", 
                htmlContent.length(), cleanContent.length());
    ...
    private int calculateReadabilityScore(Element element) {
        ...
        if (className.contains("content") || className.contains("article") || className.contains("job")) {
            score += 100;
        }
    ...
    private ExtractionConfig createDefaultConfig(String providerId) {
        ...
        filterConfig.setExcludeSelectors(List.of(
            "nav", "header", "footer", ".navigation", ".sidebar", ".ads", 
            "script", "style", ".cookie", ".social"
        ));
