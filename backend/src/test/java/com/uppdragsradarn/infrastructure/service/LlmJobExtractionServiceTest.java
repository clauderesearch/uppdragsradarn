...
/**
 * Test class for LlmJobExtractionService. Demonstrates the LLM-based extraction functionality with
 * mock data.
 */
@ExtendWith(MockitoExtension.class)
class LlmJobExtractionServiceTest {

  @Mock private OpenAiLlmService llmService;

  @Mock private HtmlContentFilterService contentFilterService;
  ...
  void testCreateAssignmentFromExtractedData() {
    ...
    when(currencyRepository.findByCode("SEK")).thenReturn(Optional.of(createSekCurrency()));
    when(skillRepository.findByNameIgnoreCase("Java")).thenReturn(Optional.of(createJavaSkill()));
    ...
    Assignment assignment =
        ReflectionTestUtils.invokeMethod(
            extractionService, "createAssignmentFromExtractedData", extractedData, jobUrl, source);
    ...
    verify(locationService)
        .processAssignmentLocation(assignment, "Stockholm, Sweden", source.getName());
  ...
  void testExtractAssignmentFromUrlWithMockData() throws Exception {
    ...
    when(contentFilterService.filterContent(any(), any())).thenReturn(mockFilteredContent);
    when(contentFilterService.estimateTokenCount(mockFilteredContent)).thenReturn(500);

    // Mock the LLM service
    when(llmService.extractAssignmentData(any(), any(), any())).thenReturn(mockExtractedData);
    ...
    System.out.println(
        "Mock filtered content token count: "
  ...
  private com.uppdragsradarn.infrastructure.crawler.config.ExtractionConfig
      createMockExtractionConfig() {
    var config = new com.uppdragsradarn.infrastructure.crawler.config.ExtractionConfig();
    config.setProviderId("emagine");

    var filterConfig =
        new com.uppdragsradarn.infrastructure.crawler.config.ExtractionConfig.ContentFilterConfig();
    filterConfig.setContentSelector(".content");
    filterConfig.setMaxTokens(2000);
    config.setContentFilter(filterConfig);

    var llmConfig =
        new com.uppdragsradarn.infrastructure.crawler.config.ExtractionConfig.LlmConfig();
  ...
  private void setupRepositoryMocks() {
    when(statusTypeRepository.findByNameAndEntityType(any(), any()))
        .thenReturn(Optional.of(createActiveStatus()));
    when(currencyRepository.findByCode(any())).thenReturn(Optional.of(createSekCurrency()));
